...
function PlayerNodeObject:_simulateEatSpore()
	if not self._mainPanel.spikySporeManager.hasSporeInVisible then
		return
	end
	local x, y = self.displayObject:GetPosition()
...
function PlayerNodeObject:_updateBluePosition()
	local globalPoint = self._mainPanel.playerContainer:ConvertToWorldSpace(self.currentPositionX * 0.001, self.currentPositionY * 0.001)
...
function PlayerNodeObject:_updateBaseInfo()
	...
	renderX = renderX * 0.001
	renderY = renderY * 0.001
	self.currentRenderX = renderX
	self.currentRenderY = renderY
end

function PlayerNodeObject:_jumpToRenderXY()
	self.displayObject:SetPosition(self.currentRenderX, self.currentRenderY)
	self.lastFrameX = self.currentRenderX
	self.lastFrameY = self.currentRenderY
...
function PlayerNodeObject:playMotion(motion, roce)
	if self.displayMode == 1 then
		return
	end
...
function PlayerNodeObject:Remove()
	...
	self.displayObject:setVisible(false)
	self.displayObject.sp:StopAllMotions()
