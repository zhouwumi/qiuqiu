...
local BBCCGridManager = import('logic.dialog.ballsbattle_cc_new.manager.bb_cc_grid_manager').BBCCGridManager

local BBCCRightUI = import('logic.dialog.ballsbattle_cc_new.ui.ballsbattle_cc_right_ui').BBCCRightUI

function Panel:init_panel()
    local native_config = g_conf_mgr.get_native_conf('ballsbattle_cc_native_data')

    constant_ballsbattle_cc.UseLuaVersion = native_config.useLua
    constant_ballsbattle_cc.RobotCnt = native_config.robotCnt
    constant_ballsbattle_cc.IsMeNetShake = native_config.isMeNetWorkShake
    constant_ballsbattle_cc.IsRobotNetShake = native_config.isOtherNetWorkShake
    constant_ballsbattle_cc.CloseGC = native_config.closeGc

    if constant_ballsbattle_cc.CloseGC then
        self.hasClose = true
        utils_enable_schedule_collect_garbage(false)
    end
    if not g_user_info.get_user_info().uid then
        g_user_info.get_user_info().uid = 100000
    end
...
function Panel:_initGame()
    ...
    self.gridManager = BBCCGridManager:New(self)
    ...
    self:get_layer():DelayCall(1, function()
        self.serverComponent:JoinPlayer(g_user_info.get_user_info().uid, constant_ballsbattle_cc.UseLuaVersion)
    end)
