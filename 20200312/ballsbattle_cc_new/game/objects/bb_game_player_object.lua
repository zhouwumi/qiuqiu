...
function BBGamePlayerObject:GetLuaCrc()
	local ret = 0
	for idx, playerNode in ipairs(self.vecPlayerNodes or {}) do
		ret = ret + playerNode:GetLuaCrc()
	end
	return ret
end
...
function BBGamePlayerObject:processTick(command)
	...
	self._mainPanel.frameManager:CacheTime(time2 - time1, time3 - time2, self:IsMe())
	self._is_node_dirty = true
	return crc;
end
