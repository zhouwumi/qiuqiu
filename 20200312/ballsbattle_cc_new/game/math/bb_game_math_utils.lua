...
function BBGameMathUtils.GetCRC32(nums, cnt)
	...
	for index = 1, cnt do
		...
		for i = 1, 4 do
			local num1 = bitband(bitShift(num, (i - 1) * 8), 0xff)
			local v1 = bitband(bitXor(crc, num1), 0xff) + 1
			local v2 = tab[v1]
			local v3 = bitXor(v2, bitShift(crc, 8))
			crc = v3
		end
	end
	if crc < 0 then
...
function BBGameMathUtils.Mass2Speed(mass)
	...
	local rate = math.floor(mass / 5) * 5 + 10
	return math.floor(8 * 1.8 * math.sqrt(20 / rate))
end
