...
function BBCCFrameManager:OnUpdate()
    ...
    local removeList, addList = self.gridManager:OnUpdateGridVisible(self.forceUpdate)
    ...
    if self.isKeyFrame then
        ...
        -- print("客户端执行时间  ", time3 - time2, self._moveTime, self._crcTime, self._nodeTimes, self._meCrcTimes)
...
function BBCCFrameManager:CacheTime(moveTime, crcTime, isMe)
    self._moveTime = self._moveTime + moveTime
    self._crcTime = self._crcTime + crcTime
    if isMe then
        self._meCrcTimes = self._meCrcTimes + crcTime
    end
    self._nodeTimes = self._nodeTimes + 1
end
