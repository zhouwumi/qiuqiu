...
function BBCCServerToClientComponent:ReStart()
    self.delay = delay_call(1 / 60, function()
        ...
        if times > 1 then
            message("连续发送了 "..times.." 帧数据")
            print("连续发送了 "..times.." 帧数据")
        end
        return 1 / 60
    end)
end
...
function BBCCServerToClientComponent:EndNewFrame()
    ...
    local isShake = math.random() < constant_ballsbattle_cc.SocketShakeRate
    if isShake then
        local shakeFrame = math.ceil(math.random() * constant_ballsbattle_cc.MaxSocketShakeFrame)
        message("网络波动了一下  "..shakeFrame.."帧")
        print("网络波动了一下  "..shakeFrame.."帧")
        delayTime = delayTime + shakeFrame * 1 / 15
    end
