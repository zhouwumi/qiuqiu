...
function BBCCBgComponent:__init__(mainPanel)
    ...
    self.nodeBoard = mainPanel.nodeBoard
    self.line_container:removeAllChildren()
    self.winW, self.winH = winSize.width, winSize.height

    self.lineW, self.lineH = self.nodeBoard.left:GetContentSize()
...
function BBCCBgComponent:_fixLineBlink()
    ...
    if math.abs(delta) > 6 then
        ...
        local size = self._mainPanel.nodeBoard.left:CalcSize(self.lineW, self.lineH)
        self._mainPanel.nodeBoard.left:setContentSize(size)
        self._mainPanel.nodeBoard.right:setContentSize(size)
        self._mainPanel.nodeBoard.top:setContentSize(size)
        self._mainPanel.nodeBoard.bottom:setContentSize(size)
    end
end
...
function BBCCBgComponent:_checkShowBoardEffect()
    ...
    if self.minLocalPoint.x > 0 and self.minLocalPoint.y > 0 and self.maxLocalPoint.x < self.lineW and self.maxLocalPoint.y < self.lineH then
        self.nodeBoard:setVisible(false)
        return
    end
    local isShow, moviesTable, moviesCount = myPlayerObject:CheckBoardHit()
    self.nodeBoard:setVisible(isShow)
    if not isShow then
        return
    end

    --添加撞击动画
    for index = 1, moviesCount do
        local moviePos = moviesTable[index]
