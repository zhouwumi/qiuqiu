...
void BBSporeManager::FixSpore(Spore* spore)
{
	...
	int fixedX, fixedY;
	BBMathUtils::FixCircle(gameManager->mapRect, point.x, point.y, spore->GetRadius(), fixedX, fixedY, isFixedX, isFixedY);
...
void BBSporeManager::MoveSpores()
{
	for (const auto& iter : mapSpores)
	{
		...
		spore->InitMove();
		spore->ChangePosition(spore->Location.x + spore->Current.x, spore->Location.y + spore->Current.y);
		FixSpore(spore);
		spore->Init -= 1;
		if (spore->Cd > 0)
		{
