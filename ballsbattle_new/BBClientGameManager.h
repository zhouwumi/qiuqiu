...
public:
	...
	void CreatePlayerFromServer(unsigned int uid, unsigned int idx, int directionX, int directionY, int NMass, int nextNodeIdx, int finalPointX, int finalPointY, bool Stopped);
	...
	void ClientSyncServerPlayerInfo(unsigned int uid, int nextNodeIdx, int nMass, int directionX, int directionY, int ackCommand, int finalPointX, int finalPointY, bool Stopped);
	...
	void AutoFillPlayerIdxInRect(int minX, int maxX, int minY, int maxY);
