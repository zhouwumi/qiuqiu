...
function RemoteScene:_caculateDirection(pos)
    ...
    if self._mainPanel.playerManager:GetMyPlayerObject() then
        ...
        cc.BBMoveManager:SetMove(angle, percent)
        -- print(self._mainPanel.frame, angle, percent) 
    end
    -- BallsBattleService.send_dirction({angle, percent})
end

function RemoteScene:SendDir()
    -- if self.currentDir then
    --     if constant_ballsbattle_cc.IsDebugBattleCC then
    --         self._mainPanel.gameManager:AddOperateMove(self._mainPanel.currentPlayerIdx, self.currentDir[1], self.currentDir[2])
    --     else
    --         local req = {self.currentDir[1], self.currentDir[2]}
    --         g_net_mgr.get_net('ballsbattlecc'):Request_CMD_SLIME_MOVE_REQ(self.currentDir[1], self.currentDir[2])
    --         self.currentDir = nil
    --         print("发送移动数据  ", req)
    --         self.lastSendDir = self.currentDir
    --     end
    --     -- self._mainPanel.gameManager:ChangePlayerDir(self._mainPanel.currentPlayerIdx, self.currentDir[1], self.currentDir[2])   
    -- end
end