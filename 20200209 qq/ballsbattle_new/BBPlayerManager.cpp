...
//client interface
void BBPlayerManager::CreatePlayerFromServer(unsigned int uid, unsigned int playerIdx, int directionX, int directionY, int NMass, int nextNodeIdx)
{
	if (gameManager->IsServer())
	{
		return;
...
void BBPlayerManager::CreatePlayerNodeFromServer(unsigned int uid, unsigned int idx, int fromId, int x, int y, int mass, int cd, int protect, int initStopFrame, int initSpeed, int initDeltaSpeed)
{
	BBPlayerNode* newPlayerNode = new BBPlayerNode();
	newPlayerNode->uid = uid;
...
void BBPlayerManager::_finishEatReleate()
{
	...
	for (auto iter : nodeId2BeEatNodes)
	{
		...
		{
			BBPlayerNode* node = mapPlayNodes[info.beEatUid];
			if (node)
			{
