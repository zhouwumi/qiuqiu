BBCCDirectorComponent = CreateClass()
local state_open = 1
local state_close = 2
...
function BBCCDirectorComponent:__init__(mainPanel)
    self.desKeys = table.keys(desInfoConfig)
    self._mainPanel = mainPanel
    self.sp = mainPanel.directorNode
    self.count = 10
    mainPanel.directorBtn.OnClick = function()
        self:_switchState()
    end
    self.messageList = {}
    ...
    if self.state == state_close then
        mainPanel.directorBtn:SetCheck(true)
    end
    self.isHigh = true
...
function BBCCDirectorComponent:OnUpdate()
...
function BBCCDirectorComponent:checkId(messageId)
...
function BBCCDirectorComponent:addSpeicalMessage(messageId)
...
function BBCCDirectorComponent:addNormalMessage()
    if self.state ~= state_open then
        return
    end
    if #self.desKeys == 0 then
        return
    end
    local times = 0
    local messageId
    while(messageId == nil)do
        local idx = math.floor(math.random(1,#self.desKeys))
        local id = self.desKeys[idx]
        messageId = self:checkId(id)
        times = times + 1
        if times >= 10 then
            return
        end
...
function BBCCDirectorComponent:_switchState()
    if self.state == state_open then
        self.messageList = {}
...
function BBCCDirectorComponent:_showMsg()
