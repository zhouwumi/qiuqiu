...
function BBGameMainGameManager:GetOtherPlayerDelay(uid)
	local player = self.playerManager:GetPlayer(uid)
	if player then
		return #player.predictionData.predictionCommands
	end
	return 0
end
...
function BBGameMainGameManager:CreateSplitPlayerNodeFromServer(uid, idx, fromId, x, y, mass, speedX, speedY, cd, protect, initStopFrame, initSpeed, initDeltaSpeed)
	self.playerManager:CreateSplitPlayerNodeFromServer(uid, idx, fromId, x, y, mass, speedX, speedY, cd, protect, initStopFrame, initSpeed, initDeltaSpeed);
end
