...
{
	BBMathUtils::BBLOG("server node %i before position before is%f-%f %f-%f, mass is %i ", idx, location.x, location.y, currentSpeedVec.x, currentSpeedVec.y, this->mass);
	...
	BBMathUtils::BBLOG("server node %i current position is %f-%f mass is %i ", idx, fixedX, fixedY, this->mass);
