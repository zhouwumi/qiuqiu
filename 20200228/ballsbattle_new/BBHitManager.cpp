...
void BBHitManager::GetCanEatFood(BBPlayerNode* playerNode, std::vector<int>& vec)
{
	...
	{
		gripRadius = BBConfigManager::protectGripRadius;
	}
...
void BBHitManager::GetCanEatSpiky(BBPlayerNode* playerNode, std::vector<int>& vec)
{
	...
	std::sort(TempVec.begin(), TempVec.end());
	double delta = BBConfigManager::ballEatRate;
	for (int i = 0; i < TempVec.size(); i++)
	{
		int idx = TempVec[i];
...
void BBHitManager::_tryEatSpore(BBPlayerNode* playerNode, Spore* targetSpore, std::vector<int>& nodeIdxs)
{
	...
	if (gameManager->frameOutManager.mapSporeIdxs.find(targetSpore->idx) != gameManager->frameOutManager.mapSporeIdxs.end())
	{
		return;
	}

	if (gameManager->frameOutManager.markSporeEat.find(targetSpore->idx) != gameManager->frameOutManager.markSporeEat.end())
	{
		return;
	}
...
{
	BBRect TempRect;
	int mass = BBMathUtils::GetRandom(BBConfigManager::minSpikyMass, BBConfigManager::maxSpikyMass);
	...
	while (!hasFind)
	{
		...
		if (gridNode.objectIdxs.size() < 2)
		{
			...
			if (!hasHit)
			{
				...
				else if (gameManager->foodSpikyManager.mapSpikyBalls.size() == 1)
				{
					x = 300;
					y = 500;
				}
				else if (gameManager->foodSpikyManager.mapSpikyBalls.size() == 2)
				{
					x = 500;
					y = 700;
				}
				else if (gameManager->foodSpikyManager.mapSpikyBalls.size() == 3)
				{
					x = 700;
					y = 700;
				}
				else if (gameManager->foodSpikyManager.mapSpikyBalls.size() == 4)
				{
					x = 500;
					y = 300;
				}
...
bool BBHitManager::FindFreeFoodPos(int& returnX, int& returnY)
{
	...
	int x = BBMathUtils::GetRandom(5, w - 5);
	int y = BBMathUtils::GetRandom(5, h - 5);
...
void BBHitManager::FindFreePlayerNodePos(int radius, int& returnX, int& returnY)
{
	...
	while (true)
	{
		...
		int x = BBMathUtils::GetRandom(1, w);
		int y = BBMathUtils::GetRandom(1, h);
		if (isFirstRandom)
		{
			isFirstRandom = false;
		}
