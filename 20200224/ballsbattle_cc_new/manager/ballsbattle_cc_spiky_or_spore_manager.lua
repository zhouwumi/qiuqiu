...
function BBCCSpikySporeManager:CreateSporeFromServer(idx)
...
function BBCCSpikySporeManager:AddSpikySporeObject(object)
    ...
    local minGridX = math.floor((x - object.radius) / self.spikyGridSize)
    minGridX = math.max(minGridX, 1)
    local minGridY = math.floor((y - object.radius) / self.spikyGridSize)
    minGridY = math.max(minGridY, 1)

    local maxGridX = math.ceil((x + object.radius) / self.spikyGridSize)
    maxGridX = math.min(maxGridX, self.spikyGridCount)
    local maxGridY = math.ceil((y + object.radius) / self.spikyGridSize)
...
function BBCCSpikySporeManager:_isObjInVisibleRect(object)
	if #self.nowRect > 0 then
		local isXInScope = object.maxGridX >= self.nowRect[1] and object.minGridX <= self.nowRect[2]
		local isYInScope = object.maxGridY >= self.nowRect[3] and object.minGridY <= self.nowRect[4]
		return isXInScope and isYInScope
	end
	return false
end

function BBCCSpikySporeManager:OnUpdateVisible(force)
