...
function BBGameMainGameManager:SyncShootFromServer(idx, fromId, uid, speedX, speedY, locationX, locationY)
	print("SyncShootFromServer  ", idx, fromId, uid, speedX, speedY, locationX, locationY)
...
function BBGameMainGameManager:GetPlayerSimpleNodeInfo(idx)
	...
	return node:GetIntLocationX(), node:GetIntLocationY(), node:GetBallMass(), node.cd,node.protect, node.currentSpeedVec.x, node.currentSpeedVec.y
end
