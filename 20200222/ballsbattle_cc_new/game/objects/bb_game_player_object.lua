...
local BBSimplePrediction = import('logic.dialog.ballsbattle_cc_new.game.data.bb_game_prediction_data').BBSimplePrediction
...
function BBGamePlayerObject:_clientOtherPrediction(forceUpdate)
	...
	if self.predictionData.lastAngle == 0 and self.predictionData.lastPressure == 0 then
		...
		local prediction = BBSimplePrediction:New()
	...
	local crc = self:processTick(predictionCommand);
	prediction.crc = crc;
	table.insert(self.predictionData.predictionCommands, prediction)
