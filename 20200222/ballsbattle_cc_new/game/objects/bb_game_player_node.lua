...
function BBGamePlayerNodeObject:GetCrc()
	return BBGameMathUtils.GetCRC32({math.floor(self.location.x), math.floor(self.location.y)})
end

function BBGamePlayerNodeObject:Move(gameManager)
	-- print("BBGamePlayerNodeObject Move before  ", self.location.x, self.location.y, self.mDeltaData.location.x, self.mDeltaData.location.y)
	local oldLocation = {x = self.location.x, y = self.location.y}
	local locVecX, locVecY = self:_stepMove()
	-- print("_stepMove ", locVecX, locVecY, self.currentSpeedVec.x, self.currentSpeedVec.y)
	...
	-- print("client node current position is: ", fixedX, fixedY, self.mass, self.mDeltaData.wrapTicks, self.player.isCatchingUp)
	if not self.player.isCatchingUp then
		...
		-- print("server node:  ", self.location.x, self.location.y, self.mDeltaData.location.x, self.mDeltaData.location.y)
	end
	gameManager.NodeTree:UpdateCircleNode(self)
	-- print("BBGamePlayerNodeObject Move after ", self.location.x, self.location.y, self.mDeltaData.location.x, self.mDeltaData.location.y)
end
		...
		if ballB.initStopFrame <= 0 and ballB ~= self then
			...
			if totalCircle > distance then
				...
				if self.cd > 0 or ballB.cd > 0 or BBGameMathUtils.NeedRollback(self, ballB, deltaLen) then
					...
					local vecX, vecY = BBGameMathUtils.GetFixedVetor2ByXY(fixedX, fixedY, length)
					if vecX == 0 and vecY == 0 then
						vecX, vecY = BBGameMathUtils.GetFixedVetor2(self.currentSpeedVec, self:GetRadius())
						...
						elseif locVecX > ballB.location.y then
							locVecY = locVecY + math.abs(vecY)
						end
					end
				end
...
function BBGamePlayerNodeObject:CalcBallDelta()
	...
	if self.player:IsMe() then
		...
		local distance = math.sqrt(deltaX * deltaX + deltaY * deltaY)
		local catchUpTicks = math.floor(distance / self.speed * 2)
		catchUpTicks = math.max(catchUpTicks, 4)
		catchUpTicks = math.min(catchUpTicks, 10)
		ticks = catchUpTicks
	end
