...
function PlayerNodeObject:_updateOtherPositionVersion2(isFirst)
	...
	if isFirst then
		...
		local x, y = self.displayObject:GetPosition()
		-- print("pass time:  ", self.objectIdx, passTime, costTime)
		...
		-- print("move delta:  ", self.objectIdx, self._mainPanel.frame, newX - x, newY - y, passTime, costTime, (newX - x) / (newY - y))
...
function PlayerNodeObject:_tryCostOtherPosition(costTime)
	...
	if self.currentCathFrame and self.currentCathFrame > 0 then
		...
		-- print('calc offsetXY  ', self.objectIdx, self.catchOffsetX, self.catchOffsetY, moveTime, self.catchOffsetX * moveTime, self.catchOffsetY * moveTime, self.currentCathFrame, self.targetFrame, self._mainPanel.frame)
