...
function RemoteScene:__init__(node)
...
function RemoteScene:_initTouch()
    local onTouchBegan = function(touch)
        return self:_onTouchBegin(touch:getLocation())
    end

    local onTouchMove = function(touch)
        self:_onTouchMove(touch:getLocation())
    end

    local onTouchEnd = function(touch)
        self:_onTouchEnd(touch:getLocation())
    end

    local onTouchCanel = function(touch)
        self:_onTouchEnd(touch:getLocation())
    end
    ...
    touchOneByOneListener:registerScriptHandler(onTouchMove, cc.Handler.EVENT_TOUCH_MOVED)
    touchOneByOneListener:registerScriptHandler(onTouchEnd, cc.Handler.EVENT_TOUCH_ENDED)
    touchOneByOneListener:registerScriptHandler(onTouchCanel, cc.Handler.EVENT_TOUCH_CANCELLED)
    self.node:getEventDispatcher():addEventListenerWithSceneGraphPriority(touchOneByOneListener, self.node)
end
...
function RemoteScene:_onTouchBegin(pos)
    ...
    -- self.node.remoteSceneSp.directionSp:setPosition(self._remoteSceneHight,self._remoteSceneHight)
...
function RemoteScene:_caculateDirection(pos)
    ...
    if x ~= 0 or y ~= 0 then
        ...
        angle = math.floor((math.deg(angle)) * 10)
        if angle == 3600 then
            angle = 0
        end
    else
