...
function PlayerNodeObject:Update()
	...
	if initStopFrame == 0 and self.initStopFrame and self.initStopFrame > 0 then
		message("init move结束")
	end
	self.initStopFrame = initStopFrame
	if initStopFrame > 0 then
		print("initStopFrame  ", playerId, idx, initStopFrame)
	end
	x = x / 1000
	y = y / 1000
	...
	print(idx, x, y, mass, renderX, renderY, lastX, lastY)
