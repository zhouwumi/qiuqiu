...
unsigned int BBPlayer::GetNextPlayerNodeIdx(bool increase)
{
	if (increase)
	{
		nextNodeIdx++;
...
bool BBPlayer::IsNeedSyncState()
{
	...
	if (gameManager->frameOutManager.sporeEatUids.find(uid) != gameManager->frameOutManager.sporeEatUids.end())
	{
		return true;
	}
	return false;
...
BBFrameData BBPlayer::serverGetFrameData()
{
	...
	ret.Stopped = Stopped;
	ret.finalPointX = BBMathUtils::bb_float_to_int(FinalPoint.x);
	ret.finalPointY = BBMathUtils::bb_float_to_int(FinalPoint.y);
...
void BBPlayer::ClientSyncServerPlayerInfo(int nextNodeIdx, int nMass, int directionX, int directionY, int ackCommand, int finalPointX, int finalPointY, bool Stopped)
{
	if (gameManager->IsServer())
	{
		return;
