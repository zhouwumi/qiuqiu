...
BBRect BBMathUtils::defaultRect(0, 0, 4500, 4500);
...
int BBMathUtils::Mass2Speed(int mass)
{
	...
	return int(floorf(8 * 1.8 * sqrtf(20.0 / rate) * BBConfigManager::massToSpeedControllerRate));
}
...
unsigned int BBMathUtils::bb_get_crc_best_value(double value)
{
	unsigned int floorValue = roundf(value);
	unsigned int ret = floor(floorValue / 5) * 5;
	/*if (value - floorValue > 0.99)
	{
		BBMathUtils::BBLOG("small difference happen... :%f ", value);
		return floorValue + 1;
	}*/
	return ret;
}
