...
int BBClientGameManager::GetOtherPlayerDelay(unsigned int uid)
{
	auto player = GetPlayer(uid);
	if (player)
	{
		return player->predictionData.predictionCommands.size();
	}
	return 0;
}
...
void BBClientGameManager::CreateSplitPlayerNodeFromServer(unsigned int uid, unsigned int idx, unsigned int fromId, int x, int y, int mass, int speedX, int speedY, int cd, int protect, int initStopFrame, int initSpeed, int initDeltaSpeed)
{
	playerManager.CreateSplitPlayerNodeFromServer(uid, idx, fromId, x, y, mass, speedX, speedY, cd, protect, initStopFrame, initSpeed, initDeltaSpeed);
}
...
std::vector<int> BBClientGameManager::SimulateEatFoods(int x, int y, int radius)
{
	std::vector<int> ret;
	std::vector<int> TempVec;
	BBRect TempPlayerRect(x - radius, y - radius, x + radius, y + radius);
	foodSpikyManager.FoodGridMap.GetAllHitIdxs(TempPlayerRect, TempVec);
	for (int i = 0; i < TempVec.size(); i++)
	{
		int idx = TempVec[i];
		auto iter = foodSpikyManager.mapFoodIdxs.find(idx);
		if (iter != foodSpikyManager.mapFoodIdxs.end())
		{
			Food* food = iter->second;

			int deltaX = x - food->location.x;
			int deltaY = y - food->location.y;
...
std::vector<int> BBClientGameManager::SimulateEatSpores(int x, int y, int radius)
{
	std::vector<int> ret;
	std::vector<int> TempVec;
	BBRect TempPlayerRect(x - radius, y - radius, x + radius, y + radius);
	...
	{
		int idx = TempVec[i];
		if (sporeManager.mapSpores.find(idx) != sporeManager.mapSpores.end())
		{
			Spore* spore = sporeManager.mapSpores[idx];
			int deltaX = x - spore->location.x;
			int deltaY = y - spore->location.y;
			if (pow(deltaX, 2) + pow(deltaY, 2) <= pow(radius, 2))
			{
				ret.emplace_back(spore->idx);
			}
		}
