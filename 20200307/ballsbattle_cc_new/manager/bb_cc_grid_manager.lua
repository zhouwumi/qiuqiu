...
function BBCCGridManager:OnUpdateGridVisible(force)
	...
	local minGridX = math.floor(minLocalPoint.x / self.gridSize) - 1
	minGridX = math.max(minGridX, 1)
	local maxGridX = math.ceil(maxLocalPoint.x  / self.gridSize) + 1
	maxGridX = math.min(maxGridX, self.gridCount)

	local minGridY = math.floor(minLocalPoint.y / self.gridSize) - 1
	minGridY = math.max(minGridY, 1)
	local maxGridY = math.ceil(maxLocalPoint.y / self.gridSize) + 1
