...
{
	BBMathUtils::BBLOG("server node current position before is %f-%f %f-%f, mass is %i ", location.x, location.y, currentSpeedVec.x, currentSpeedVec.y, this->mass);
	...
	BBMathUtils::BBLOG("server node current position is %i, %f-%f mass is %i ", idx, fixedX, fixedY, this->mass);
		...
		if (ballB->initStopFrame <= 0 && ballB != this)
		{
			...
			if (totalCircle > distance)
			{
				...
				if (this->cd > 0 || ballB->cd > 0 || BBMathUtils::NeedRollback(this, ballB, deltaLen))
				{
					...
					if (vec == BBVector::ZERO)
					{
						vec = BBVector::GetFixedVetor2(currentSpeedVec, this->GetRadius());
						locVec.x -= vec.x;
						locVec.y -= vec.y;
					}
					else
...
void BBPlayerNode::EatFoodSpikySporeChangeMass(BBGameManager* gameManager)
{
	...
	if (deltaMass != 0)
	{
		...
		if (this->GetBallMass() + deltaMass < BBConfigManager::initBallMass)
		{
			deltaMass = BBConfigManager::initBallMass - this->GetBallMass();
		}
...
void BBPlayerNode::CalcBallDelta()
{
	...
	else 
	{
		...
		int catchTicks = floorf(distance / speed * 2);
		if (catchTicks > 10)
		{
			catchTicks = 10;
		}
