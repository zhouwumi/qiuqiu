...
void BBPlayerManager::SetPlayerProperty(int uid, bool isBalance)
{
	Player* player = mapPlayers[uid];
	if (!player)
	{
...
void BBPlayerManager::PlayerSplit(int uid)
{
	...
	for (int idx : selectedNodes)
	{
		...
		if (gameManager->IsServer())
		{
			...
			float childMas = child->mass / 2.0f;
			child->ChangeMass(-childMas);
		}
	}
}
...
{
	int childMas = sourceNode->mass / 2;
	...
	newPlayerNode->CalculateInitMoveParams(newPlayerNode->GetCircleRadius(), BBConst::SplitFrame, BBConst::SplitInitSpeed, BBConst::SplitFinalSpeed);
...
void BBPlayerManager::MovePlayer(Player* player)
{
	...
	if (player->IsBalance && allPlayerNodes.size() > 1 && player->Direction.y == 0)
	{
		return;
	}
	...
	for (int i = 0; i < allPlayerNodes.size(); i++)
	{
		...
		{
			node->isHit = _handleNodeHit(player, node, locVec);
			if (node->isHit)
			{
	...
	if (player->Direction.y == 0 && isBalance && allPlayerNodes.size() > 1 && isAllHit)
	{
		player->IsBalance = true;
	}
}
...
void BBPlayerManager::MakePhoto(int keyFrame)
{
	for (int i = 0; i < playerIds.size(); i++)
	{
		auto iter = mapPlayers.find(playerIds[i]);
		...
		{
			Player* player = (*iter).second;
			player->MakePhoto(keyFrame);
		}
	}
}

void BBPlayerManager::ResetPhoto(int keyFrame)
{
	for (int i = 0; i < playerIds.size(); i++)
	{
		auto iter = mapPlayers.find(playerIds[i]);
...
void BBPlayerManager::CopyPlayerManager(BBPlayerManager* otherPlayerManager)
{
	for (int i = 0; i < otherPlayerManager->playerIds.size(); i++)
	{
		int playerId = otherPlayerManager->playerIds[i];
		this->playerIds.emplace_back(playerId);
		Player* otherPlayer = otherPlayerManager->mapPlayers[playerId];
		Player* player = gameManager->objectManager.CreatePlayer(playerId);
		mapPlayers.emplace(playerId, player);
		player->CopyFromOtherPlayer(otherPlayer);
		for (int j = 0; j < player->vecPlayerNodes.size(); j++)
		{
			PlayerNode* playerNode = player->vecPlayerNodes[j];
			mapPlayNodes.emplace(playerNode->Idx, playerNode);
		}
	}
}