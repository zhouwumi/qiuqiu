...
{
	playerManager.MakePhoto(serverKeyFrame);
	playerManager.KeyFrameUpdatePlayer();
	sporeManager.MakePhoto(serverKeyFrame);
}
...
{
	playerManager.ResetPhoto(serverKeyFrame);
	sporeManager.ResetPhoto(serverKeyFrame);
}
...
void BBGameManager::SetPlayerProperty(int uid, bool isBalance)
{
	playerManager.SetPlayerProperty(uid, isBalance);
}
...
void BBGameManager::CopyFromGameManager(BBGameManager* copyGameManager)
{
	this->gameFrame = copyGameManager->gameFrame;
	this->playerManager.CopyPlayerManager(&(copyGameManager->playerManager));
}