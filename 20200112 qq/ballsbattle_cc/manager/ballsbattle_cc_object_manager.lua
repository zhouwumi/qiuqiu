...
function BBCCObjectManager:CreateFoodWithServerInfo(foodInfo)
	...
	local clientLocation = BBCCUtils.ClientPackLocation(x, y)
	self._gameManager:AddNewFoodFromServer(clientLocation)
    local foodObj = FoodObject:New(self._mainPanel)
    foodObj:SetPosition(x, y, clientLocation)
    local idx = self._gameManager:GetFoodIdxByPos(clientLocation)
    self.foodMap[idx] = foodObj
...
function BBCCObjectManager:GenerateFoodAndSpiky()
	local allFoods = self._gameManager:GetAllFoodInfos()
	for _, value  in ipairs(allFoods) do
	    local x = value % 4500
	    local y = (value - x ) / 4500
	    local foodObj = FoodObject:New(self._mainPanel)
	    foodObj:SetPosition(x, y, value)
	    local idx = self._gameManager:GetFoodIdxByPos(value)
	    self.foodMap[idx] = foodObj
	    self._foodManager:AddFood(foodObj)
	end

	self.spikyMap = {}
	local allSpikys = self._gameManager:GetAllSpikyInfos()
	for _, value in ipairs(allSpikys) do
	    local x, y, radius = self._gameManager:GetSpikyInfo(value)
	    local spikyObject = SpikyObject:New(self._mainPanel)
	    spikyObject:SetPosition(x, y, radius, value)
	    self.spikyMap[value] = spikyObject
	    self._spikySporeManager:AddSpikyObject(spikyObject)
	end
