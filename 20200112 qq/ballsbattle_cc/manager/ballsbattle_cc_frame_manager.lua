...
function BBCCFrameManager:__init__(mainPanel)
	...
	self.actionNode:DelayCall(0.5, function()
...
function BBCCFrameManager:OnUpdateWithSimulate()
	...
	if self.lastSimulateEndFrame and self.lastSimulateEndFrame > curCFrame then
		print("OnUpdateWithSimulate  ", self.lastSimulateEndFrame, curCFrame)
		self.lastSimulateEndFrame = self.lastSimulateEndFrame + 1
		runTimes = self.lastSimulateEndFrame - curCFrame
	end
	...
	if self.stepFrame == 5 then
		if #self.frameDatas > 0 then
			if self.simulateFrame >= 0 then --有关键帧数据,需要一步一步撤销模拟数据
				...
				self.lastSimulateEndFrame = self.currentServerFrame + 5 + self.simulateFrame
				self.simulateFrame = -1
			else
				self:DoKeyFrame()
			end
