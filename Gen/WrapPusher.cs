...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<Language>(translator.PushLanguage, translator.Get, translator.UpdateLanguage);
        ...
        int Language_TypeID = -1;
		int Language_EnumRef = -1;
        
        public void PushLanguage(RealStatePtr L, Language val)
        {
            if (Language_TypeID == -1)
            {
			    bool is_first;
                Language_TypeID = getTypeId(L, typeof(Language), out is_first);
				
				if (Language_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(Language));
				    Language_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, Language_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, Language_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for Language ,value="+val);
            }
			
			LuaAPI.lua_getref(L, Language_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out Language val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != Language_TypeID)
				{
				    throw new Exception("invalid userdata for Language");
				}
                ...
                {
                    throw new Exception("unpack fail for Language");
                }
				val = (Language)e;
        ...
        public void UpdateLanguage(RealStatePtr L, int index, Language val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for Language ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			{
			    Language[] array = obj as Language[];
				translator.PushLanguage(L, array[index]);
				return true;
			}
            return false;
		}
